...
sha256sum tmp.txt | awk '{print $1}' | xargs -I {} curl -X 'POST' 'http://192.168.0.101:39080/asr?task=translate&language=ko&output=srt' -H 'accept: application/json' -H 'Content-Type: multipart/form-data' -F 'audio_file=@{}.mp3;type=audio/mpeg' > "$(awk '{$1=$2=""; print $0}' tmp.txt | sed "s/^[ \t]*//").srt"
rm GoogleTranslate.py
g++ trans_modi.cpp -o trans_modi.o && ./trans_modi.o
node srtx.js "$(awk '{$1=$2=""; print $0}' tmp.txt | sed "s/^[ \t]*//").srt"
echo "===yt-Whisper-srt=== (srt length ADJ...)"
g++ srtn.cpp -o srtn.o && ./srtn.o "$(awk '{$1=$2=""; print $0}' tmp.txt | sed "s/^[ \t]*//").srttx" | tee -i utf8 "$(awk '{$1=$2=""; print $0}' tmp.txt | sed "s/^[ \t]*//")_lengthADJ.srt"
